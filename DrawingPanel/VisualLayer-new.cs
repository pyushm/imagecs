...
namespace ImageProcessor
{
    ...
    public class VisualLayer : Viewport3D//UIElement
    {   // collection of visual items comprizing one UIElement (similar to DrawingGroup, but working with DependencyProperties)
        ...
        public int AddVisual(Visual v) { return visualChildren.Add(v); }
        public void RemoveVisual(Visual v) { visualChildren.Remove(v); }
        public void Clear() { visualChildren.Clear(); }
        ...
        protected override Size MeasureOverride(Size s) { return LayoutSize.Size; }
    ...
    public class BitmapLayer : VisualLayer  // layers of this type shown on the layer list
    {
        ...
        public void RedrawImage()
        {
            ...
            visualChildren.Add(CreateImageDrawing());
    ...
    public class Bitmap3DLayer : BitmapLayer
    {
        public Bitmap3DLayer(string name, BitmapAccess ba, int edge) : base(name)
        {
            try
            {
                ...
                LayoutSize = new IntSize(ba.Width, ba.Height);
                SetImage(ba, edge);
                //SetProjection(image.Source, 0.2, 0);
                visualChildren.Clear();
                visualChildren.Add(CreateImageDrawing());
                //Effect = new ColorAdjustmentEffect();
                SetEffectParameters(0, 0, 1);
        ...
        public void SetProjection(BitmapSource imageSource, double distortionX, double distortionY)
        {
            ...
            Camera = new PerspectiveCamera(new Point3D(0, 0, cameraDistance), new Vector3D(0, 0, -1), new Vector3D(0, 1, 0), angleOfView);
            Width = w;
            Height = h;
            ...
            Children.Add(i3d);
            Children.Add(new ModelVisual3D() { Content = new AmbientLight(Colors.White) });
        }
