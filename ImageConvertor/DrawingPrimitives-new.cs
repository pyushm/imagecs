...
namespace ImageProcessor
{
    ...
    {   // Mouse action indexes: 0-3 - control points: TL=0, TR=1, BR=2, BL=3; 4-7 control lines: Top=4, Right=5, Bottom=6, Left=7  
        ...
        Distortion = -8,       // 3d view distortion 
    }
    ...
    {   // matrix is represented as a product of rotation (R), shear (H), scale (S), and aspect (A) matrixes
        ...
        public double SetShearAndAspect(MouseOperations mouseAction, Vector d)
        {
            int mouseActionInd = (int)mouseAction;
            if (mouseActionInd<0)
                return 0;
        ...
        public bool ApplyShearAndAspectIncrement(MouseOperations mouseAction, double inc)
        {
            int mouseActionInd = (int)mouseAction;
            if (mouseActionInd < 0)
                return false;
    ...
    {
        public Vector ViewDistortion { get; set; }
        public override Vector SetViewDistortion(Point p) { return ViewDistortion = (p - Center)/ quadrantSize; }
        Point distortionPos { get { return quadrantSize * ViewDistortion + Center; } } // distortion indicator location in canvas coordinates
        public override MouseOperations OperationFromPoint(Point p)  // finds MouseOperation from point
        {
            if ((p - distortionPos).LengthSquared < 2 * mSize * mSize)
                return MouseOperations.Distortion;
